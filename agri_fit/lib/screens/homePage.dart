...
class _HomePageState extends State<HomePage> {
  static DateTime getDateDaysPast(int days) {
    return DateTime.now().subtract(Duration(days: days));
  } 

  Future<List<int>> getWeeklySteps() async {
    return [
      await _requestStepsWeek(getDateDaysPast(32), getDateDaysPast(25)),
      await _requestStepsWeek(getDateDaysPast(24), getDateDaysPast(17)),
      await _requestStepsWeek(getDateDaysPast(16), getDateDaysPast(9)),
      await _requestStepsWeek(getDateDaysPast(8), getDateDaysPast(1)),
    ];
  }
                ...
                child: ListView(children: [
                  ...
                  SizedBox(
                      ...
                      child: Container(
                          ...
                          child: FutureBuilder(
                              future: getWeeklySteps(),
                              builder: (BuildContext context,
                                  AsyncSnapshot<List<int>?> snapshot) {
                                //print(snapshot);
                                if (snapshot.hasData) {
                                  return StepsGraph(
                                    weeklySteps: snapshot.data!,
                      );
                                } else {
                        ...
                        child: Center(
                          ...
                          children: [
                            ...
                            FutureBuilder<List<Steps>?>(
                              ...
                              builder: (BuildContext context,
                                  AsyncSnapshot<List<Steps>?> snapshot) {
                                //print(snapshot);
                                if (snapshot.hasData) {
                        ...
                        child: Center(
                          child: Column(children: [
                            ...
                            FutureBuilder<List<Callories>?>(
                              ...
                              builder: (BuildContext context,
                                  AsyncSnapshot<List<Callories>?> snapshot) {
                                print(snapshot);
                                if (snapshot.hasData) {
                                  return Text(snapshot.data!
                                      .map((e) => e.cals)
  ...
  Future<int> _requestStepsWeek(
    ...
    if (response.statusCode == 200) {
      ...
      for (var i = 0; i < decodedResponse['data'].length; i++) {
        //i fra 0 til 4
        String sDate = decodedResponse['data'][0]['date'];
        String eDate = decodedResponse['data'].last['date'];
        print(sDate);
        for (var j = 0; j < decodedResponse['data'][i]['data'].length; j++) {
          ...
          Map<String, dynamic> json = decodedResponse['data'][i]['data'][j];
    ...
    return result.map((e) => e.step).reduce((a, b) => a + b);
  }
